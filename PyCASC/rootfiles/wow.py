    ...
    def __init__( self, f: BytesIO ):
        ...
        if len( dat ) == 12:
            (
                ...
                self.content_flags,
                self.locale_flags
            ) = struct.unpack( "3I", dat )
        else:
            self.valid = False


class WOWROOT_Group:
    def __init__( self, f: BytesIO, root_format, handler ):
        ...
        if root_format == WOWROOT_FORMAT_82:
            ...
            if self.header.content_flags & 0x10000000 == 0:
                self.hashes = [ struct.unpack( "Q", f.read( 8 ) ) for x in range( self.header.number_of_files ) ]
...
class WOWROOT_Handler:
    def __init__( self, f: BytesIO, root_format, hashless_count ):
        self.f              = f
